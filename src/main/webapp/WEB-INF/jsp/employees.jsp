...
<link rel="stylesheet" type="text/css" href="<c:url value='/styles/employee.css"'/>
...
<fieldset>
	...
	<form name="filterEmps" action="searchemps" method="get">
		<label>Name:</label><input type="text" name="filterName" id="filterBox"/>
...
<table id="t1"> 
	<tr><!-- <sec:authorize access="hasRole('ROLE_ADMIN')"> -->
		...
		<th>Employee Name</th> <th>Deptartment</th> <th>Job Title</th>
	</tr> 
	<c:forEach items="${emps}" var="emp">
		<tr id="tableRow-${emp.id}"> 
			...
			<div class="editBtn-containerClass"
					id="editBtn-container-${emp.id}">
					<button type="button" class="editBtnClass"
						id="editBtn-${emp.id}">Edit</button>
				</div>
			</td>
			<td>
	...
	<fieldset>
		...
		<form name="editEmp" action="updateEmp" id="editForm-${emp.id}" method="post">
		<input type="hidden" name="id" value="${emp.id}" />
		<input type="hidden" id="activeVal-${emp.id}" name="isActive" value="true" />
		<div><label>First Name: *</label><input type="text" name="firstName" value="${emp.firstName}" required/>
		<label>Last Name: *</label><input type="text" name="lastName" value="${emp.lastName}" required/>
		<label>Middle Initial:</label><input type="text" name="middleInitial" value="${emp.middleInitial}" maxlength="1" size="1"/>
			<label>Department:</label>
			<select name="department.id">
				<c:forEach items="${depts}" var="dept">
					<c:choose>
			...
			<br><br>
			<label>Email: *</labeL><input type="text" name="email" value="${emp.email}" required/>
			...
			<br><br>
			<button type="submit">Save</button>
			<button type="button" class="deleteBtnClass" id="deleteBtn-${emp.id}">Delete</button>
			<button type="reset" class="cancelEditBtnClass" id="cancelEditBtn-${emp.id}"value="reset">Cancel</button>
		</div>
		<br><div>Required Fields indicated with a *</div>
		</form>
	</fieldset>
