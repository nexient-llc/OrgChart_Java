...
public class DepartmentServiceTest {
	...
	public void before() throws Exception {
		...
		when(this.mockRepository.findAll(new Sort(Direction.ASC, "name")))
				.thenReturn(this.listOfFoundDepts);
		when(this.mockRepository.findOne(Entities.DEPT_ID)).thenReturn(
				this.mockDepartment);
		when(this.mockRepository.save(this.mockDepartment)).thenReturn(
				this.mockDepartment);
		this.departmentService.setRepository(this.mockRepository);
	}

	@Test
	public void findAllDepartments() {
		List<Department> depts = this.departmentService.findAll();
		assertNotNull(depts);
